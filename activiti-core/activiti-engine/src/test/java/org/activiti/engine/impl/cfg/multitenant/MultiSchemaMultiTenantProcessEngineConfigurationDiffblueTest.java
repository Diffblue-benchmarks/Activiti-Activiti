...
public class MultiSchemaMultiTenantProcessEngineConfigurationDiffblueTest {
  ...
  public void testBuildProcessEngine() {
    ...
    DynamicBpmnService dynamicBpmnService = actualBuildProcessEngineResult.getDynamicBpmnService();
    assertTrue(dynamicBpmnService instanceof DynamicBpmnServiceImpl);
    HistoryService historyService = actualBuildProcessEngineResult.getHistoryService();
    assertTrue(historyService instanceof HistoryServiceImpl);
    ManagementService managementService = actualBuildProcessEngineResult.getManagementService();
    ...
    RepositoryService repositoryService = actualBuildProcessEngineResult.getRepositoryService();
    assertTrue(repositoryService instanceof RepositoryServiceImpl);
    RuntimeService runtimeService = actualBuildProcessEngineResult.getRuntimeService();
    assertTrue(runtimeService instanceof RuntimeServiceImpl);
    TaskService taskService = actualBuildProcessEngineResult.getTaskService();
    assertTrue(taskService instanceof TaskServiceImpl);
    CommandExecutor commandExecutor = ((DynamicBpmnServiceImpl) dynamicBpmnService).getCommandExecutor();
    assertTrue(commandExecutor instanceof CommandExecutorImpl);
    assertEquals("default", actualBuildProcessEngineResult.getName());
    assertEquals(HistoryLevel.AUDIT, multiSchemaMultiTenantProcessEngineConfiguration.getHistoryLevel());
    assertTrue(multiSchemaMultiTenantProcessEngineConfiguration.booted);
    assertSame(multiSchemaMultiTenantProcessEngineConfiguration,
        actualBuildProcessEngineResult.getProcessEngineConfiguration());
    assertSame(commandExecutor, ((HistoryServiceImpl) historyService).getCommandExecutor());
    assertSame(commandExecutor, ((ManagementServiceImpl) managementService).getCommandExecutor());
    assertSame(commandExecutor, ((RepositoryServiceImpl) repositoryService).getCommandExecutor());
    assertSame(commandExecutor, ((RuntimeServiceImpl) runtimeService).getCommandExecutor());
    assertSame(commandExecutor, ((TaskServiceImpl) taskService).getCommandExecutor());
  }
  ...
  public void testBuildProcessEngine2() {
    ...
    multiSchemaMultiTenantProcessEngineConfiguration
        .setSqlSessionFactory(new DefaultSqlSessionFactory(new Configuration()));
    multiSchemaMultiTenantProcessEngineConfiguration.addConfigurator(new ProcessExecutionLoggerConfigurator());
    multiSchemaMultiTenantProcessEngineConfiguration.setDatabaseType("none");
    multiSchemaMultiTenantProcessEngineConfiguration.registerTenant("42", mock(DataSource.class));
