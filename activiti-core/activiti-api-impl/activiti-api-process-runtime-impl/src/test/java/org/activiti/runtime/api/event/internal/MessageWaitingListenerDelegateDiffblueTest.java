...
class MessageWaitingListenerDelegateDiffblueTest {
   ...
   *
   * <ul>
   *   <li>{@link MessageWaitingListenerDelegate#MessageWaitingListenerDelegate(List,
   *       ToMessageWaitingConverter)}
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({
    "void MessageWaitingListenerDelegate.<init>(List, ToMessageWaitingConverter)",
    "boolean MessageWaitingListenerDelegate.isFailOnException()"
  })
  void testGettersAndSetters() {
    // Arrange
    ArrayList<BPMNElementEventListener<BPMNMessageWaitingEvent>> processRuntimeEventListeners =
        new ArrayList<>();

    // Act
    MessageWaitingListenerDelegate actualMessageWaitingListenerDelegate =
        new MessageWaitingListenerDelegate(
            processRuntimeEventListeners,
            new ToMessageWaitingConverter(new BPMNMessageConverter()));

    // Assert
    assertFalse(actualMessageWaitingListenerDelegate.isFailOnException());
  }
   ...
   * </ul>
   *
   * <p>Method under test: {@link MessageWaitingListenerDelegate#onEvent(ActivitiEvent)}
   */
  @Test
  @DisplayName(
  ...
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void MessageWaitingListenerDelegate.onEvent(ActivitiEvent)"})
  void testOnEvent_givenBPMNElementEventListenerOnEventDoesNothing_thenCallsOnEvent() {
    // Arrange
    BPMNElementEventListener<BPMNMessageWaitingEvent> bpmnElementEventListener =
    ...
    MessageWaitingListenerDelegate messageWaitingListenerDelegate =
        new MessageWaitingListenerDelegate(
            processRuntimeEventListeners,
            new ToMessageWaitingConverter(new BPMNMessageConverter()));
    ...
    // Act
    messageWaitingListenerDelegate.onEvent(event);
