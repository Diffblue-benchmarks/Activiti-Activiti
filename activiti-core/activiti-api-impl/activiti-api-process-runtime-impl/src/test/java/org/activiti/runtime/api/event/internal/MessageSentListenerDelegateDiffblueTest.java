...
class MessageSentListenerDelegateDiffblueTest {
   ...
   *
   * <ul>
   *   <li>{@link MessageSentListenerDelegate#MessageSentListenerDelegate(List,
   *       ToMessageSentConverter)}
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({
    "void MessageSentListenerDelegate.<init>(List, ToMessageSentConverter)",
    "boolean MessageSentListenerDelegate.isFailOnException()"
  })
  void testGettersAndSetters() {
    // Arrange
    ArrayList<BPMNElementEventListener<BPMNMessageSentEvent>> processRuntimeEventListeners =
        new ArrayList<>();

    // Act
    MessageSentListenerDelegate actualMessageSentListenerDelegate =
        new MessageSentListenerDelegate(
            processRuntimeEventListeners, new ToMessageSentConverter(new BPMNMessageConverter()));

    // Assert
    assertFalse(actualMessageSentListenerDelegate.isFailOnException());
  }
   ...
   * </ul>
   *
   * <p>Method under test: {@link MessageSentListenerDelegate#onEvent(ActivitiEvent)}
   */
  @Test
  @DisplayName(
  ...
  void testOnEvent_givenBPMNElementEventListenerOnEventDoesNothing_thenCallsOnEvent() {
    // Arrange
    BPMNElementEventListener<BPMNMessageSentEvent> bpmnElementEventListener =
    ...
    MessageSentListenerDelegate messageSentListenerDelegate =
        new MessageSentListenerDelegate(
            processRuntimeEventListeners, new ToMessageSentConverter(new BPMNMessageConverter()));
