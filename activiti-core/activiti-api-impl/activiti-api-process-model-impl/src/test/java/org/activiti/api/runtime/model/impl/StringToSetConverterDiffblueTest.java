...
class StringToSetConverterDiffblueTest {
  ...
  @MethodsUnderTest({"Set StringToSetConverter.convert(String)"})
  void testConvertWithString() {
    // Arrange, Act and Assert
    assertThrows(RuntimeException.class,
        () -> (new StringToSetConverter(JsonMapper.builder().findAndAddModules().build())).convert("Source"));
  }
  ...
  void testConvertWithString2() {
    ...
    // Act and Assert
    assertThrows(RuntimeException.class,
        () -> (new StringToSetConverter(builderResult.findAndAddModules().build())).convert("Source"));
  ...
  void testConvertWithString_givenLRUMapGetReturnNull_whenSource_thenCallsPutIfAbsent() {
    ...
    verify(cacheProvider).forSerializerCache(isNull());
    verify(cacheProvider).forTypeFactory();
    verify(lruMap, atLeast(1)).get(Mockito.<Object>any());
    verify(lruMap, atLeast(1)).putIfAbsent(Mockito.<Object>any(), Mockito.<JavaType>any());
  }
   ...
   * Test {@link StringToSetConverter#convert(String)} with {@code String}.
   * <ul>
   *   <li>Given {@link ObjectMapper} {@link ObjectMapper#readValue(String, JavaType)} return {@link HashSet#HashSet()}.</li>
  ...
  void testConvertWithString_when42() {
    // Arrange, Act and Assert
    assertThrows(RuntimeException.class,
        () -> (new StringToSetConverter(JsonMapper.builder().findAndAddModules().build())).convert("42"));
  }
  ...
  void testConvertWithString_whenEmptyString() {
    // Arrange, Act and Assert
    assertThrows(RuntimeException.class,
        () -> (new StringToSetConverter(JsonMapper.builder().findAndAddModules().build())).convert(""));
  }
}
