...
class StringToMapConverterDiffblueTest {
  ...
  void testConvertWithString6() {
    ...
    when(defaultImpl.isEnumType()).thenReturn(true);
    when(defaultImpl.getContentType()).thenReturn(new PlaceholderForType(1));
    when(defaultImpl.getKeyType()).thenReturn(new PlaceholderForType(1));
    when(defaultImpl.isContainerType()).thenReturn(true);
    when(defaultImpl.isAbstract()).thenReturn(true);
    when(defaultImpl.isMapLikeType()).thenReturn(true);
    Class<Object> forNameResult = Object.class;
    Mockito.<Class<?>>when(defaultImpl.getRawClass()).thenReturn(forNameResult);
    PlaceholderForType bt = new PlaceholderForType(1);
    PlaceholderForType baseType = new PlaceholderForType(1);
    TypeFactory typeFactory = TypeFactory.defaultInstance();
    ...
    DeserializationConfig config =
        new DeserializationConfig(
            base, str, mixins, rootNames, configOverrides, new CoercionConfigs(), null);

    AsDeductionTypeDeserializer src =
        new AsDeductionTypeDeserializer(bt, idRes, defaultImpl, config, new ArrayList<>());
    AsDeductionTypeDeserializer asDeductionTypeDeserializer =
        new AsDeductionTypeDeserializer(src, new Bogus());

    StdTypeResolverBuilder typer2 = mock(StdTypeResolverBuilder.class);
    when(typer2.buildTypeDeserializer(
            Mockito.<DeserializationConfig>any(),
            Mockito.<JavaType>any(),
            Mockito.<Collection<NamedType>>any()))
        .thenReturn(asDeductionTypeDeserializer);
    Class<Object> forNameResult2 = Object.class;
    Mockito.<Class<?>>when(typer2.getDefaultImpl()).thenReturn(forNameResult2);

    Builder builderResult2 = JsonMapper.builder();
    builderResult2.setDefaultTyping(typer2);
    builderResult2.addHandler(mock(DeserializationProblemHandler.class));
    JsonMapper objectMapper = builderResult2.findAndAddModules().build();
    ...
    assertThrows(
        RuntimeException.class, () -> new StringToMapConverter(objectMapper).convert("42"));
    verify(defaultImpl, atLeast(1)).getContentType();
    verify(defaultImpl, atLeast(1)).getKeyType();
    verify(defaultImpl, atLeast(1)).getRawClass();
    verify(defaultImpl).isAbstract();
    verify(defaultImpl, atLeast(1)).isContainerType();
    verify(defaultImpl).isEnumType();
    verify(defaultImpl, atLeast(1)).isMapLikeType();
    verify(typer2, atLeast(1))
        .buildTypeDeserializer(
            isA(DeserializationConfig.class),
            Mockito.<JavaType>any(),
            Mockito.<Collection<NamedType>>any());
    verify(typer2, atLeast(1)).getDefaultImpl();
  }
   ...
   *
   * <ul>
   *   <li>Given {@link JavaType} {@link JavaType#getContentType()} throw {@link
  ...
  @Test
  @DisplayName(
      "Test convert(String) with 'String'; given JavaType getContentType() throw RuntimeException()")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"Map StringToMapConverter.convert(String)"})
  void testConvertWithString_givenJavaTypeGetContentTypeThrowRuntimeException() {
    // Arrange
    JavaType defaultImpl = mock(JavaType.class);
    when(defaultImpl.getContentType()).thenThrow(new RuntimeException());
    when(defaultImpl.isContainerType()).thenReturn(true);
    Class<Object> forNameResult = Object.class;
    Mockito.<Class<?>>when(defaultImpl.getRawClass()).thenReturn(forNameResult);
    PlaceholderForType bt = new PlaceholderForType(1);
    ...
    assertThrows(
        RuntimeException.class, () -> new StringToMapConverter(objectMapper).convert("42"));
    verify(defaultImpl).getContentType();
    verify(defaultImpl).getRawClass();
    verify(defaultImpl).isContainerType();
    verify(typer2, atLeast(1))
        .buildTypeDeserializer(
            isA(DeserializationConfig.class),
            Mockito.<JavaType>any(),
   ...
   *
   * <ul>
   *   <li>Given {@link JavaType} {@link JavaType#isConcrete()} return {@code true}.
   *   <li>Then calls {@link JavaType#isArrayType()}.
   * </ul>
   *
   * <p>Method under test: {@link StringToMapConverter#convert(String)}
   */
  @Test
  @DisplayName(
      "Test convert(String) with 'String'; given JavaType isConcrete() return 'true'; then calls isArrayType()")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"Map StringToMapConverter.convert(String)"})
  void testConvertWithString_givenJavaTypeIsConcreteReturnTrue_thenCallsIsArrayType() {
    // Arrange
    ArrayType arrayType = mock(ArrayType.class);
    when(arrayType.getValueHandler()).thenReturn("Value Handler");

    JavaType defaultImpl = mock(JavaType.class);
    when(defaultImpl.isConcrete()).thenReturn(true);
    when(defaultImpl.isRecordType()).thenReturn(true);
    when(defaultImpl.isArrayType()).thenReturn(true);
    when(defaultImpl.isEnumType()).thenReturn(true);
    when(defaultImpl.getContentType()).thenReturn(arrayType);
    when(defaultImpl.getKeyType()).thenReturn(new PlaceholderForType(1));
    when(defaultImpl.isContainerType()).thenReturn(true);
    when(defaultImpl.isAbstract()).thenReturn(true);
    when(defaultImpl.isMapLikeType()).thenReturn(true);
    Class<Map> forNameResult = Map.class;
    Mockito.<Class<?>>when(defaultImpl.getRawClass()).thenReturn(forNameResult);
    PlaceholderForType bt = new PlaceholderForType(1);
    PlaceholderForType baseType = new PlaceholderForType(1);
    TypeFactory typeFactory = TypeFactory.defaultInstance();
    ...
    verify(defaultImpl).isAbstract();
    verify(defaultImpl, atLeast(1)).isArrayType();
    verify(defaultImpl).isConcrete();
    verify(defaultImpl, atLeast(1)).isContainerType();
    verify(defaultImpl).isEnumType();
    verify(defaultImpl, atLeast(1)).isMapLikeType();
    verify(defaultImpl, atLeast(1)).isRecordType();
    verify(typer2, atLeast(1))
        .buildTypeDeserializer(
            isA(DeserializationConfig.class),
            Mockito.<JavaType>any(),
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"Map StringToMapConverter.convert(String)"})
  void testConvertWithString_thenCallsGetValueHandler() {
    // Arrange
    ArrayType arrayType = mock(ArrayType.class);
    when(arrayType.getValueHandler()).thenReturn("Value Handler");
