...
class DemoApplicationDiffblueTest {
  ...
  void testRun_givenArrayListAddProcessDefinitionImpl_thenCallsProcessDefinitions() {
    ...
    // Act
    (new DemoApplication(processRuntime, null, securityUtil)).run("Args");
   ...
   * <ul>
   *   <li>Given {@link ProcessRuntime} {@link ProcessRuntime#start(StartProcessPayload)} return {@link ProcessInstanceImpl} (default constructor).</li>
   *   <li>Then calls {@link ProcessRuntime#start(StartProcessPayload)}.</li>
   * </ul>
   * <p>
   * Method under test: {@link DemoApplication#processText()}
   */
  @Test
  @DisplayName("Test processText(); given ProcessRuntime start(StartProcessPayload) return ProcessInstanceImpl (default constructor); then calls start(StartProcessPayload)")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"void DemoApplication.processText()"})
  void testProcessText_givenProcessRuntimeStartReturnProcessInstanceImpl_thenCallsStart() {
    // Arrange
    ProcessRuntime processRuntime = mock(ProcessRuntime.class);
    ...
    // Act
    (new DemoApplication(processRuntime, null, securityUtil)).processText();
  ...
  void testCheckAndWorkOnTasksWhenAvailable() {
    ...
    // Act
    (new DemoApplication(null, taskRuntime, securityUtil)).checkAndWorkOnTasksWhenAvailable();
  ...
  void testCheckAndWorkOnTasksWhenAvailable3() {
    ...
    ArrayList<VariableInstance> variableInstanceList = new ArrayList<>();
    variableInstanceList.add(new VariableInstanceImpl<>("content", "bob",
        new Content("Not all who wander are lost", true, new ArrayList<>()), "42", "42"));
    TaskRuntime taskRuntime = mock(TaskRuntime.class);
  ...
  void testCheckAndWorkOnTasksWhenAvailable4() {
    ...
    ArrayList<VariableInstance> variableInstanceList = new ArrayList<>();
    variableInstanceList.add(
        new VariableInstanceImpl<>("content", "bob", new Content("activiti", true, new ArrayList<>()), "42", "42"));
    TaskRuntime taskRuntime = mock(TaskRuntime.class);
    when(taskRuntime.complete(Mockito.<CompleteTaskPayload>any()))
        .thenReturn(new TaskImpl("42", "Name", TaskStatus.CREATED));
    when(taskRuntime.variables(Mockito.<GetTaskVariablesPayload>any())).thenReturn(variableInstanceList);
  ...
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"void DemoApplication.checkAndWorkOnTasksWhenAvailable()"})
  void testCheckAndWorkOnTasksWhenAvailable5() {
    // Arrange
    ArrayList<Task> content = new ArrayList<>();
    content.add(new TaskImpl("42", "bob", TaskStatus.CREATED));
    PageImpl<Task> pageImpl = new PageImpl<>(content, 1000);
    ...
    doNothing().when(content2).setApproved(anyBoolean());
    when(content2.getBody()).thenReturn("Not all who wander are lost");
    VariableInstanceImpl<Object> variableInstanceImpl = new VariableInstanceImpl<>("content", "bob", content2, "42",
        "42");
    ...
    verify(taskRuntime).tasks(isA(Pageable.class));
    verify(taskRuntime).variables(isA(GetTaskVariablesPayload.class));
    verify(content2).getBody();
    verify(content2).setApproved(eq(false));
    verify(securityUtil).logInAs(eq("bob"));
  }
   ...
   * Test {@link DemoApplication#checkAndWorkOnTasksWhenAvailable()}.
   * <ul>
   *   <li>Given {@link Content} {@link Content#getBody()} return {@code activiti}.</li>
   * </ul>
   * <p>
   * Method under test: {@link DemoApplication#checkAndWorkOnTasksWhenAvailable()}
   */
  @Test
  @DisplayName("Test checkAndWorkOnTasksWhenAvailable(); given Content getBody() return 'activiti'")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"void DemoApplication.checkAndWorkOnTasksWhenAvailable()"})
  void testCheckAndWorkOnTasksWhenAvailable_givenContentGetBodyReturnActiviti() {
    // Arrange
    ArrayList<Task> content = new ArrayList<>();
    content.add(new TaskImpl("42", "bob", TaskStatus.CREATED));
    PageImpl<Task> pageImpl = new PageImpl<>(content, 1000);
    ...
    Content content2 = mock(Content.class);
    doNothing().when(content2).setApproved(anyBoolean());
    when(content2.getBody()).thenReturn("activiti");
    VariableInstanceImpl<Object> variableInstanceImpl = new VariableInstanceImpl<>("content", "bob", content2, "42",
        "42");
    ...
    verify(taskRuntime).variables(isA(GetTaskVariablesPayload.class));
    verify(content2).getBody();
    verify(content2).setApproved(eq(true));
    verify(securityUtil).logInAs(eq("bob"));
  }
